...
class GDImage {
	 ...
	 * 			$img -> resize(120, 300);
	 */
	function resize($new_width, $new_height, $zc=true, $use_resize = false) {
		if(!$this->image) return false;
		...
		if($zc) {
	        ...
	        if($use_resize) imagecopyresized($new_image, $this->image, 0, 0, $src_x, $src_y, $new_width, $new_height, $src_w, $src_h);
			else imagecopyresampled($new_image, $this->image, 0, 0, $src_x, $src_y, $new_width, $new_height, $src_w, $src_h);
		} else {
	 ...
	 * 			$image->save('file.jpg');
	 */
	function save($file_name, $quality=90, $destroy = true) {
		if(!$this->image) return false;
		...
		switch($extension) {
			case 'png' :
				...
				return imagepng($this->image, $file_name, $quality);
				break;
			case 'jpeg': 
			case 'jpg' :
